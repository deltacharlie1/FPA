		...
		$faq_tag = preg_replace('~&#x0*([0-9a-f]+);~ei', 'chr(hexdec("\\1"))', $faq_tag);
			...
			$displayfaq= '<div class="faq_list">';
				...
				$displayfaq .= '<div class="single_faq">';
					if(get_option('faq_jquery') == 'true') {
				$displayfaq .= '<h3 class="faq_question click">'.get_the_title().'</h3>';
					} else {
				$displayfaq .= '<h3 class="faq_question">'.get_the_title().'</h3>';
					}
				$displayfaq .= '<span class="faq_answer">'.wpautop($faqcontent, true).'</span>';
				$displayfaq .= '</div>';
				endwhile;
				wp_reset_query();
			$displayfaq .= '</div>';
...
function faqlist_shortcode($atts, $content = NULL) {
		extract(shortcode_atts(array(
			'faq_topic'	=> '',
			'faq_tag'	=> '',
			'faq_id'	=> '',
		...
		$faq_tag = preg_replace('~&#x0*([0-9a-f]+);~ei', 'chr(hexdec("\\1"))', $faq_tag);
			...
			$displayfaq= '<div class="faq_list">';
				...
				global $post;
				$displayfaq .= '<div class="single_faq">';
				$displayfaq .= '<h3 class="faqlist_question"><a href="'.get_permalink().'" title="Permanent link to '.get_the_title().'" >'.get_the_title().'</a></h3>';
				$displayfaq .= '</div>';
				endwhile;
			wp_reset_query();
			$displayfaq .= '</div>';
