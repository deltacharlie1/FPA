...
(function($) {
  ...
  $.fn.pekeUpload = function(options){
    ...
    var defaults = {
      ...
      allowedExtensions:  "",
      invalidExtError:    "Resim Seçin Lütfen",
      maxSize:      0,
      sizeError:      "Çok Büyük Bir Resim Seçtiniz",
    ...
    function UploadFile(){
      ...
      formData.append('form',$(options.form).serialize());
      $.ajax({
            url: options.url,
            type: 'POST',
            data: formData,
...
//        }
      ...
      fileCount++;
      if (options.limit>0) {
        var validationresult = validateLimit();
        if (validationresult == false){
          canUpload = false;
          if((options.theme == "bootstrap")&&(options.showErrorAlerts==true)){
            obj.next('a').next('div').prepend('<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button> '+options.limitError+'</div>');
            bootstrapclosenotification();
          }
          if((options.theme == "custom")&&(options.showErrorAlerts==true)){
            obj.next('a').next('div').prepend('<div class="alert-pekeupload"><button type="button" class="close">&times;</button> '+options.limitError+'</div>');
            customclosenotification();
          }
          options.onFileError(file,options.limitError);
        }
      }
      return canUpload
    }
