...
unless ($FORM{rows}) {
	$Customers = $dbh->prepare("select id,cusname,cusaddress,cuspostcode,cusregion,custel,cuscontact,cusbalance,cuscredit,cuslimit,cusdefpaymethod from customers where cusname $SQL and acct_id=? and cussales='Y' and cusbalance>0");
	$Customers->execute("$COOKIE->{ACCT}");
	$FORM{numrows} = $Customers->rows;
	$FORM{offset} = 0;
	$FORM{rows} = 24;
}
$Customers = $dbh->prepare("select id,cusname,cusaddress,cuspostcode,cusregion,custel,cuscontact,cusbalance,cuscredit,cuslimit,cusdefpaymethod,cussuppress from customers where cusname $SQL and acct_id=? and cussales='Y' and cusbalance>0 order by cusname limit $FORM{offset},$FORM{rows}");
$Customers->execute("$COOKIE->{ACCT}");
...
function redisplay(action) {
  ...
  location.href = "/cgi-bin/fpa/list_debtors.pl?numrows=" + numrows + "&offset=" + offset + "&rows=" + rows + "&search=" + srch;
}
