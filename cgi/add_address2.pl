...
#  Change default expenses category (defcoa) if supplier
	...
	if ($FORM{id} > 0) {	#  An Update
		...
		$Sts = $dbh->do("insert into customers (acct_id,cusname,cusaddress,cuspostcode,cusregion,custel,cuscontact,cusemail,custerms,cusdefpo,cusbank,cussortcode,cusacctno,cusbalance,cuslimit,cussales,cussupplier,cusremarks,cusdefpaymethod,cusdefvatrate,cusemailmsg,cusstmtmsg,cusautostmts,cuscis,cuslayout,cusdeliveryaddr,cussuppress) values ('$COOKIE->{ACCT}','$FORM{cusname}','$FORM{cusaddress}','$FORM{cuspostcode}','$FORM{cusregion}','$FORM{custel}','$FORM{cuscontact}','$FORM{cusemail}','$FORM{custerms}','$FORM{cusdefpo}','$FORM{cusbank}','$FORM{cussortcode}','$FORM{cusacctno}','0','$FORM{cuslimit}','$FORM{cussales}','$FORM{cussupplier}','$FORM{cusremarks}','$FORM{cusdefpaymethod}','$FORM{cusdefvatrate}','$FORM{cusemailmsg}','$FORM{cusstmtmsg}','$FORM{cusautostmts}','$FORM{cuscis}','$FORM{cuslayout}','$FORM{cusdeliveryaddr}',$FORM{cussuppress}')");
