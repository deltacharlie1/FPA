...
if ($Layout_id > 0) {
	...
	($PDF_data,$Invoice_no) = &pdf_invoicel($Invoice_id,$PDFStamp,$Tplt,$Layout_id,$Testonly);
}
else {
	require "/usr/local/httpd/cgi-bin/fpa/pdf_invoice.ph";
	($PDF_data,$Invoice_no) = &pdf_invoice($Invoice_id,$PDFStamp,$Tplt,$Layout_id,$Testonly);
}
