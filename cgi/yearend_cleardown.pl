...
$Dates = $dbh->prepare("select date_sub(comyearend,interval 1 year) as tbend,date_sub(date_add(comyearend, interval 1 day), interval 2 year) as tbstart,date_sub(date_add(comyearend, interval 1 day), interval 1 year) as fystart,date_format(date_sub(comyearend,interval 1 year),'%d-%b-%y') as dispend,date_format(date_sub(date_add(comyearend, interval 1 day), interval 2 year),'%d-%b-%y') as dispstart,date_format(date_sub(comyearend,interval 1 year),'%d-%b-%y') as repend,date_format(date_sub(date_add(comyearend, interval 1 day), interval 2 year),'%d-%b-%y') as repstart from companies where reg_id=$Reg_id and id=$Com_id");
$Dates->execute;
