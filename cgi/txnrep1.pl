...
#  script to display the transactions report input screen
...
$Vars = {
        title => 'Accounts - Search Transactions',
	cookie => $COOKIE,
	focus => 'invcusname',
...
function get_results(action) {
  ...
  $.get("/cgi-bin/fpa/txnrep2.pl",$("form#form1").serialize() ,function(data) {
