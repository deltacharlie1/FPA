...
$Events_summary .=   "======\t\t======\t\t=========\t========\n";
$Ev_sum_len = length($Events_summary);
...
#  Find the gcl record for this subscription and update the payment ID
		...
		$Sts = $dbh->do("update companies set comsubdue=date_add('$Event->{created_at}',interval 37 day) where compayref='$Event->{links}->{payment}'");
		$Events_summary .= "$Sts\t\tConfirmed\t$Event->{links}->{subscription}\t\t$Event->{links}->{payment}\n";
		if ($Sts > 0) {
...
if (length($Events_summary) > $Ev_sum_len) {
